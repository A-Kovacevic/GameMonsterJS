...
function begin(){
	ctxBG.drawImage(imgSprite,0,0, canvasWidth, canvasHeight, 0,0, canvasWidth, canvasHeight);  //background
	isPlaying=true; //begin
...
Player.prototype.checkDirection=function (){
	...
	else if(this.isRightKey){ //without else we can use up and right together
		newDrawX+=this.speed;
...
Player.prototype.checkObstacleCollide=function(newDrawX, newDrawY){
	...
	if(obstaclesCounnter === obstacles.length){
		return false;
	}else{
		return true;
	}
...
function Enemy(){
	...
	this.width=45;
	this.height=54;
	this.drawX= rangomRange(0, canvasWidth - this.width); 
	this.drawY=rangomRange(0, canvasHeight-this.height);
	this.centerX= this.drawX +(this.width/2);
	this.centerY= this.drawY + (this.height/2);
	this.targetX=this.centerX;
	this.targetY=this.centerY;
	this.randomMoveTime =rangomRange(4000,10000);
	this.speed=1;
	var that=this;
...
Enemy.prototype.setTargetLocation=function(){
	...
	this.targetX=rangomRange(minX, maxX);
	this.targetY=rangomRange(minY,maxY);
